(function() {
  ...
  window.ApsaraModules.push({
    ...
    function gambarPath(entry) {
      return `./aset/gambar_berian/${entry.id}_${entry.konsep.replace(/[^a-zA-Z0-9]/g, '_').replace(/_+/g, '_').replace(/_$/, '').toLowerCase()}.webp`;
    }

    // Hitung berapa banyak gambar yang benar-benar tersimpan di cache offline.
    // Mengembalikan { cached, total } agar status belum/sebagian/semua bisa dibedakan.
    async function hitungTersimpan() {
      const data = window.BERIAN_DATA || [];
      const total = data.length;
      try {
        const cacheKeys = await caches.keys();
        const assetCacheName = cacheKeys.find(k => k.startsWith('apsara-asset-'));
        if (!assetCacheName) return { cached: 0, total };
        const cache = await caches.open(assetCacheName);
        const tersimpan = new Set((await cache.keys()).map(r => r.url));
        let cached = 0;
        for (const entry of data) {
          if (tersimpan.has(new URL(gambarPath(entry), location.href).href)) cached++;
        }
        return { cached, total };
      } catch (e) { return { cached: 0, total }; }
    }

    if (!window.Capacitor && window.location.protocol !== 'file:') {
      setTimeout(async () => {
        const { cached, total } = await hitungTersimpan();
        if (cached < total) tombol.classList.add('perlu-unduh-kedip');
      }, 2000);
    }
    ...
    tombol.addEventListener('click', () => {
      ...
      overlay.innerHTML = `<div class="modal" style="position:relative; max-width:500px; padding:24px;">
        ...
        <div id="cacheStatus" style="${window.Capacitor || window.location.protocol === 'file:' ? 'display:none;' : ''}margin-top:8px; font-size:11px; color:#64748b; text-align:center; line-height:1.5;">Memeriksa penyimpanan…</div>
      </div>`;
      document.body.appendChild(overlay);

      const btnCacheImages = overlay.querySelector('#btnCacheImages');
      const cacheStatus = overlay.querySelector('#cacheStatus');

      // Perbarui label status & teks tombol sesuai jumlah gambar yang tersimpan.
      async function perbaruiStatus() {
        const info = await hitungTersimpan();
        const { cached, total } = info;
        if (cacheStatus) {
          if (cached === 0) {
            cacheStatus.textContent = `Belum ada gambar tersimpan (0 / ${total}).`;
            cacheStatus.style.color = '#ef4444';
          } else if (cached < total) {
            cacheStatus.textContent = `Tersimpan sebagian: ${cached} / ${total} gambar.`;
            cacheStatus.style.color = '#f6921e';
          } else {
            cacheStatus.textContent = `✓ Semua gambar tersimpan (${total} / ${total}).`;
            cacheStatus.style.color = '#16a34a';
          }
        }
        if (btnCacheImages) {
          if (cached === 0) btnCacheImages.textContent = 'Unduh Gambar Offline';
          else if (cached < total) btnCacheImages.textContent = `Lanjutkan Unduh (${total - cached} tersisa)`;
          else btnCacheImages.textContent = 'Unduh Ulang Semua';
        }
        if (cached >= total && total > 0) tombol.classList.remove('perlu-unduh-kedip');
        else tombol.classList.add('perlu-unduh-kedip');
        return info;
      }
      ...
      if (btnCacheImages) {
        ...
        btnCacheImages.addEventListener('click', async () => {
          ...
          const { cached, total } = await hitungTersimpan();
          const tersisa = total - cached;
          if (tersisa === 0) {
            if (!confirm(`Semua ${total} gambar sudah tersimpan. Unduh ulang semuanya?`)) return;
          } else if (!confirm(`Akan mengunduh ${tersisa} gambar${cached > 0 ? ` yang belum tersimpan (dari total ${total})` : ''} ke penyimpanan offline. Proses ini memakan waktu dan kuota internet. Lanjutkan?`)) {
            return;
          }
          ...
          try {
            ...
            // Unduh ulang penuh memaksa semua; selain itu hanya yang belum tersimpan.
            const unduhUlangPenuh = tersisa === 0;
            const daftar = window.BERIAN_DATA.filter(entry =>
              unduhUlangPenuh || !tersimpan.has(new URL(gambarPath(entry), location.href).href)
            );

            let success = 0, fail = 0;
            const totalProses = daftar.length;
            const batchSize = 10;
            for (let i = 0; i < totalProses; i += batchSize) {
              const batch = daftar.slice(i, i + batchSize);
              await Promise.all(batch.map(async (entry) => {
                const imgPath = gambarPath(entry);
                try {
              ...
              if (cacheStatus) {
                cacheStatus.style.color = '#334155';
                cacheStatus.textContent = `Mengunduh… ${Math.round(((i + batch.length) / totalProses) * 100)}% (${success} berhasil${fail ? `, ${fail} gagal` : ''})`;
              }
            }

            const info = await perbaruiStatus();
            if (fail > 0) {
              alert(`Selesai sebagian: ${info.cached} / ${info.total} gambar tersimpan. ${fail} gambar gagal diunduh — periksa koneksi internet lalu tekan "Lanjutkan Unduh".`);
            } else {
              alert(`✓ Berhasil! ${info.cached} / ${info.total} gambar tersimpan offline.`);
            }
          } catch (e) {
            alert('Gagal mengunduh gambar: ' + e.message);
          } finally {
            btnCacheImages.disabled = false;
            await perbaruiStatus();
          }
        });
      }
